...
from home_tile_reporting_etl_Pipeline_1 import HomeTileReportingETL
...
class TestHomeTileReportingETL:
    ...
    def test_etl_initialization_without_spark(self):
        ...
        with patch('home_tile_reporting_etl_Pipeline_1.SparkSession.getActiveSession') as mock_spark:
            mock_session = Mock()
            mock_spark.return_value = mock_session
            etl = HomeTileReportingETL()
            assert etl.spark == mock_session
    
    @patch('home_tile_reporting_etl_Pipeline_1.SOURCE_HOME_TILE_EVENTS', 'test.tile_events')
    @patch('home_tile_reporting_etl_Pipeline_1.SOURCE_INTERSTITIAL_EVENTS', 'test.interstitial_events')
    @patch('home_tile_reporting_etl_Pipeline_1.SOURCE_TILE_METADATA', 'test.metadata')
    ...
    @patch.object(HomeTileReportingETL, 'write_delta')
    def test_run_etl_success(self, mock_write_delta, mock_create_global_kpis, mock_create_daily_summary,
                           mock_enrich_with_metadata, mock_aggregate_interstitial_events,
    ...
    def test_data_type_validation(self, etl_instance, spark_session):
        ...
        assert isinstance(schema_dict['unique_tile_views'], (IntegerType, type(None)))  # May be LongType
        assert isinstance(schema_dict['unique_tile_clicks'], (IntegerType, type(None)))  # May be LongType
